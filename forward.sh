...
sudo iptables -F
sudo iptables -t nat -p tcp -A PREROUTING  -s $src_ip  -d $zt_ip --match multiport   ! --dports 8000,5201  -j DNAT --to $xbox_ip
sudo iptables -t nat -p udp -A PREROUTING  -s $src_ip  -d $zt_ip --match multiport   ! --dport 5102        -j DNAT --to $xbox_ip
